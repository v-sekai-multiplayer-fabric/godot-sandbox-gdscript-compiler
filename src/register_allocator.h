 ...
 *
 * Maintains a pool of 18 free physical registers (t0-t6, s1-s11, excluding
 * s0/fp) and maps virtual registers to physical RISC-V registers using hash
 * maps.
 *
 * Algorithm:
 * - When allocating a register for a virtual register, checks if already
 * allocated (return it), spilled to stack (load it), or new (take from free
 * pool).
 * - If the free pool is empty, uses the "Furthest Next Use" heuristic to select
 *   a spill candidate: picks the virtual register whose next use is furthest
 * away (or unknown/never used again).
 * - This keeps values needed soon in registers and spills values not needed
 * soon, improving code quality with minimal complexity.
 *
 * The allocator tracks all uses of each virtual register in sorted lists,
 * allowing efficient binary search to find the next use after any given
 * instruction.
 *
...
public:
   ...
   * @param current_instr_idx Current instruction index (used for next use
   * computation)
  ...
  handle_syscall_clobbering(const std::vector<uint8_t> &clobbered_regs,
   ...
   *
   * Removes the mapping from vreg to physical register and frees the physical
   * register. This is necessary when the value is updated on the stack through
   * variant operations, to prevent stale physical register values from being
   * used.
   *
  ...
  /**
   * @brief Force a virtual register to be mapped to a specific physical
   * register
   *
   * This is used for RELOAD instructions where we need to ensure a value is in
   * a specific register (e.g., REG_T0) regardless of the allocator's state.
   ...
   */
  const std::vector<uint8_t> &get_available_registers() const {
    return m_free_registers;
  }
...
private:
  ...
  static constexpr uint8_t REG_S0 =
      8; // x8 (fp) - excluded, used as frame pointer
   ...
   *
   * Allows quick lookup of which virtual register is in a given physical
   * register.
   */
  ...
  /**
   * @brief All uses of each virtual register (for computing next use
   * dynamically)
   *
   ...
   * "Furthest Next Use" improvement over simple "first allocated = first
   * spilled".
   *
   * @param current_instr_idx Current instruction index
