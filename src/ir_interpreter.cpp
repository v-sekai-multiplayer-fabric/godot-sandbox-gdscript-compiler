...
IRInterpreter::Value IRInterpreter::call(const std::string &function_name,
...
void IRInterpreter::execute_function(const IRFunction &func,
                                     ExecutionContext &ctx) {
  ctx.pc = 0;
...
IRInterpreter::Value IRInterpreter::get_register(ExecutionContext &ctx,
                                                 int reg) {
...
void IRInterpreter::execute_instruction(const IRInstruction &instr,
                                        ExecutionContext &ctx) {
  switch (instr.opcode) {
  ...
  case IROpcode::CMP_GTE: {
    ...
    int src1 = std::get<int>(instr.operands[1].value);
    int src2 = std::get<int>(instr.operands[2].value);
    ctx.registers[dst] = compare_op(get_register(ctx, src1),
                                    get_register(ctx, src2), instr.opcode);
    break;
  }
  ...
  case IROpcode::AND: {
    ...
    int src2 = std::get<int>(instr.operands[2].value);
    bool result =
        get_bool(get_register(ctx, src1)) && get_bool(get_register(ctx, src2));
  ...
  default:
    throw CompilerException(ErrorType::OPTIMIZER_ERROR,
                            "Unimplemented opcode in interpreter");
  }
}
...
IRInterpreter::Value IRInterpreter::binary_op(const Value &left,
                                              const Value &right, IROpcode op) {
...
IRInterpreter::Value IRInterpreter::unary_op(const Value &operand,
                                             IROpcode op) {
  switch (op) {
  case IROpcode::NEG:
...
IRInterpreter::Value
IRInterpreter::compare_op(const Value &left, const Value &right, IROpcode op) {
  int64_t l = get_int(left);
  int64_t r = get_int(right);
