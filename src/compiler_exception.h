...
public:
   ...
   */
  CompilerException(ErrorType error_type, const std::string &message,
                    int line = 0, int column = 0,
                    const std::string &function = "",
                    const std::string &file = "",
                    const std::string &source_line = "",
                    const std::string &hint = "")
      : m_error_type(error_type), m_message(message), m_file(file),
        m_line(line), m_column(column), m_function(function),
        m_source_line(source_line), m_hint(hint) {}

  // Getters

  ErrorType error_type() const { return m_error_type; }
  const char *error_type_string() const {
    return error_type_to_string(m_error_type);
  }
    ...
    if (!m_source_line.empty()) {
      ...
      if (m_column > 0) {
        ...
        for (int i = 1; i < m_column; ++i) {
          ...
          if (static_cast<size_t>(i - 1) < m_source_line.length() &&
              m_source_line[i - 1] == '\t') {
            oss << "    ";
          } else {
            oss << " ";
  ...
  static CompilerException lexer_error(const std::string &message, int line,
                                       int column) {
    return CompilerException(ErrorType::LEXER_ERROR, message, line, column);
  }

  static CompilerException parser_error(const std::string &message, int line,
                                        int column) {
    return CompilerException(ErrorType::PARSER_ERROR, message, line, column);
  }

  static CompilerException semantic_error(const std::string &message, int line,
                                          int column,
                                          const std::string &function = "") {
    return CompilerException(ErrorType::SEMANTIC_ERROR, message, line, column,
                             function);
  }

  static CompilerException codegen_error(const std::string &message,
                                         const std::string &function = "") {
    return CompilerException(ErrorType::CODEGEN_ERROR, message, 0, 0, function);
  }

  static CompilerException
  riscv_codegen_error(const std::string &message,
                      const std::string &function = "") {
    return CompilerException(ErrorType::RISCV_codegen_ERROR, message, 0, 0,
                             function);
  }

  static CompilerException
  undefined_variable(const std::string &var_name, int line, int column,
                     const std::string &function = "") {
    return CompilerException(
        ErrorType::SEMANTIC_ERROR, "Undefined variable: " + var_name, line,
        column, function, "", "",
        "Make sure '" + var_name + "' is declared before use");
  }

  static CompilerException type_error(const std::string &message, int line,
                                      int column,
                                      const std::string &function = "") {
    return CompilerException(ErrorType::SEMANTIC_ERROR,
                             "Type error: " + message, line, column, function);
  }

  static CompilerException syntax_error(const std::string &message, int line,
                                        int column) {
    return CompilerException(ErrorType::PARSER_ERROR,
                             "Syntax error: " + message, line, column);
  }
};
...
#define THROW_COMPILER_ERROR(error_type, message)                              \
  throw gdscript::CompilerException((error_type), (message), __LINE__, 0,      \
                                    __func__, __FILE__)
