...
void IROptimizer::optimize_function(IRFunction &func) {
  ...
  // Run peephole again to catch patterns that emerged after previous
  // optimizations
  peephole_optimization(func);
  ...
  // Run peephole once more after eliminate_redundant_stores to clean up
  // remaining patterns
  peephole_optimization(func);
...
void IROptimizer::constant_folding(IRFunction &func) {
  ...
  for (const auto &instr : func.instructions) {
    ...
    // Important: invalidate all constants when we encounter control flow
    // targets because we don't know what path we took to reach this point
    ...
    case IROpcode::DIV:
    case IROpcode::MOD: {
      // Check that all operands are registers before attempting constant
      // folding
      ...
      if (m_constants.count(lhs_reg) && m_constants.count(rhs_reg)) {
        ...
        if (try_fold_binary_op(instr.opcode, instr.type_hint,
                               m_constants[lhs_reg], m_constants[rhs_reg],
                               result)) {
          // Replace with appropriate load instruction based on result type
          if (result.type == ConstantValue::Type::FLOAT) {
            new_instructions.emplace_back(IROpcode::LOAD_FLOAT_IMM,
                                          IRValue::reg(dst),
                                          IRValue::fimm(result.float_value));
            new_instructions.back().type_hint = Variant::FLOAT;
          } else {
            new_instructions.emplace_back(IROpcode::LOAD_IMM, IRValue::reg(dst),
      ...
      if (m_constants.count(lhs_reg) && m_constants.count(rhs_reg)) {
        ...
        if (try_fold_binary_op(instr.opcode, instr.type_hint,
          ...
          new_instructions.emplace_back(
              IROpcode::LOAD_BOOL, IRValue::reg(dst),
    ...
    case IROpcode::NOT: {
      ...
      if (m_constants.count(src) &&
          m_constants[src].type == ConstantValue::Type::BOOL) {
        ConstantValue result;
        result.type = ConstantValue::Type::BOOL;
        result.bool_value = !m_constants[src].bool_value;
        new_instructions.emplace_back(IROpcode::LOAD_BOOL, IRValue::reg(dst),
                                      IRValue::imm(result.bool_value ? 1 : 0));
        set_register_constant(dst, result);
        folded = true;
      }
    ...
    case IROpcode::OR: {
      ...
      if (m_constants.count(lhs_reg) && m_constants.count(rhs_reg)) {
        ...
        if (lhs_cv.type == ConstantValue::Type::BOOL &&
            rhs_cv.type == ConstantValue::Type::BOOL) {
          ConstantValue result;
          result.type = ConstantValue::Type::BOOL;
...
bool IROptimizer::try_fold_binary_op(IROpcode op,
                                     IRInstruction::TypeHint type_hint,
                                     const ConstantValue &lhs,
                                     const ConstantValue &rhs,
                                     ConstantValue &result) {
  // Handle float arithmetic - if type_hint is VARIANT_FLOAT or either operand
  // is float, we must perform float arithmetic (GDScript semantics)
  bool is_float_op =
      (type_hint == Variant::FLOAT || lhs.type == ConstantValue::Type::FLOAT ||
  ...
  if (is_float_op) {
    ...
    double lhs_val = (lhs.type == ConstantValue::Type::FLOAT)
                         ? lhs.float_value
                         : static_cast<double>(lhs.int_value);
    double rhs_val = (rhs.type == ConstantValue::Type::FLOAT)
                         ? rhs.float_value
  ...
  // Handle integer arithmetic (only when both operands are int and not a float
  // operation)
  if (!is_float_op && lhs.type == ConstantValue::Type::INT &&
      rhs.type == ConstantValue::Type::INT) {
    switch (op) {
    case IROpcode::ADD:
      result.type = ConstantValue::Type::INT;
  ...
  bool comparable = (lhs.type == ConstantValue::Type::INT &&
                     rhs.type == ConstantValue::Type::INT) ||
                    (lhs.type == ConstantValue::Type::FLOAT &&
                     rhs.type == ConstantValue::Type::FLOAT) ||
                    (lhs.type == ConstantValue::Type::INT &&
                     rhs.type == ConstantValue::Type::FLOAT) ||
                    (lhs.type == ConstantValue::Type::FLOAT &&
                     rhs.type == ConstantValue::Type::INT);
  ...
  if (comparable) {
    ...
    if (lhs_is_float || rhs_is_float) {
      // Float comparison
      double lhs_val =
          lhs_is_float ? lhs.float_value : static_cast<double>(lhs.int_value);
      double rhs_val =
          rhs_is_float ? rhs.float_value : static_cast<double>(rhs.int_value);
...
void IROptimizer::peephole_optimization(IRFunction &func) {
  ...
  while (i < func.instructions.size()) {
    ...
    // Transform to: BRANCH_CMP lhs, rhs, label (where CMP is the comparison
    // type) This eliminates the need to store and load the comparison result
    ...
    if (!skip && !handled && i + 1 < func.instructions.size()) {
      ...
      bool is_branch_on_cmp =
          (branch_instr.opcode == IROpcode::BRANCH_ZERO ||
           branch_instr.opcode == IROpcode::BRANCH_NOT_ZERO);

      if (is_cmp && is_branch_on_cmp && cmp_instr.operands.size() >= 3 &&
          branch_instr.operands.size() >= 2) {
        ...
        // Check if branch uses the comparison result AND result is not used
        // elsewhere We check if it's used after the branch (i+2 onwards), not
        // including the branch itself Pass false for conservative_at_labels
        // since comparison results don't persist across control flow
        bool reg_not_used_after = !is_reg_used_between_exclusive(
            func, cmp_dst, i + 1, func.instructions.size(), false);
        if (cmp_dst == branch_reg && reg_not_used_after) {
          ...
          // Map CMP + BRANCH_ZERO to direct branch (or invert for
          // BRANCH_NOT_ZERO)
          if (invert) {
            // BRANCH_ZERO means branch when condition is false, so invert
            // comparison
            ...
            default:
              throw CompilerException(
                  ...
                  "Unexpected comparison opcode in peephole optimization");
            }
          } else {
            // BRANCH_NOT_ZERO means branch when condition is true, use same
            // comparison
            switch (cmp_instr.opcode) {
            case IROpcode::CMP_EQ:
              fused_opcode = IROpcode::BRANCH_EQ;
    ...
    if (!skip && !handled && instr.opcode == IROpcode::MOVE) {
      ...
      // Pattern 2: Eliminate redundant MOVEs around arithmetic/logical
      // operations Pattern A: MOVE tmp1, src1; MOVE tmp2, src2; OP dst, tmp1,
      // tmp2; MOVE result, dst
      //          -> OP result, src1, src2
      ...
      if (!skip && !handled && i + 3 < func.instructions.size()) {
        ...
        if (move2.opcode == IROpcode::MOVE && is_arithmetic_op(op.opcode) &&
            move3.opcode == IROpcode::MOVE && op.operands.size() >= 3) {
          ...
          if (op.operands[1].type == IRValue::Type::REGISTER &&
            ...
            if (move1_dst == op_lhs && move2_dst == op_rhs &&
              ...
              bool tmp1_safe =
                  !is_reg_used_between_exclusive(func, move1_dst, i, i + 2);
              bool tmp2_safe =
                  !is_reg_used_between_exclusive(func, move2_dst, i + 1, i + 2);
              bool dst_safe =
                  !is_reg_used_between_exclusive(func, op_dst, i + 2, i + 3);
      ...
      if (!skip && !handled && i + 2 < func.instructions.size()) {
        ...
        if (is_arithmetic_op(op.opcode) && move2.opcode == IROpcode::MOVE &&
            op.operands.size() >= 3) {
      ...
      // Pattern E: MOVE tmp, var; LOAD_IMM/LOAD_FLOAT_IMM const; OP dst, tmp,
      // const; MOVE var, dst
      //          -> OP var, var, const
      ...
      if (!skip && !handled && i + 3 < func.instructions.size()) {
        ...
        if (move1.opcode == IROpcode::MOVE &&
            (load.opcode == IROpcode::LOAD_IMM ||
             load.opcode == IROpcode::LOAD_FLOAT_IMM) &&
            is_arithmetic_op(op.opcode) && move2.opcode == IROpcode::MOVE &&
            op.operands.size() >= 3) {
          ...
          // Check if operands match the pattern: MOVE tmp, var; LOAD const; OP
          // dst, tmp, const; MOVE var, dst
          if (op.operands[1].type == IRValue::Type::REGISTER &&
              op.operands[2].type == IRValue::Type::REGISTER) {
            int op_lhs = std::get<int>(op.operands[1].value);
            int op_rhs = std::get<int>(op.operands[2].value);

            // Pattern E requires: tmp=move1_dst=op_lhs, const=load_dst=op_rhs,
            // var=move1_src=move2_dst, dst=op_dst=move2_src
            if (move1_dst == op_lhs && load_dst == op_rhs &&
                move1_src == move2_dst && move2_src == op_dst) {
              // Check that temps are not used elsewhere (excluding the OP
              // instruction at i+2) tmp1 (move1_dst) should only be used by OP
              // at i+2 - check before OP
              bool tmp1_safe =
                  !is_reg_used_between_exclusive(func, move1_dst, i, i + 2);
              // const (load_dst) should only be used by OP at i+2 - check
              // before and after (excluding OP)
              bool tmp2_safe =
                  !is_reg_used_between_exclusive(func, load_dst, i + 1,
                                                 i + 2) &&
                  !is_reg_used_between_exclusive(func, load_dst, i + 2, i + 3);
              // dst (op_dst) should only be used by MOVE at i+3 - check between
              // OP and MOVE
              bool dst_safe =
                  !is_reg_used_between_exclusive(func, op_dst, i + 2, i + 3);

              if (tmp1_safe && tmp2_safe && dst_safe) {
                // Emit the LOAD_IMM instruction first (we still need the
                // constant)
                ...
                IRInstruction new_op = op;
                new_op.operands[0] =
                    IRValue::reg(move2_dst); // var as destination
                new_op.operands[1] =
                    IRValue::reg(move1_src); // var as first operand
    ...
    // Pattern F: Handle MOVE r10, r0; MOVE r0, r10 -> eliminate (redundant
    // pair)
    if (!skip && !handled && i + 1 < func.instructions.size()) {
      if (func.instructions[i].opcode == IROpcode::MOVE &&
        ...
        if (move1_dst == move2_src && move1_src == move2_dst &&
            move1_dst != move1_src) {
...
void IROptimizer::copy_propagation(IRFunction &func) {
  // This optimization eliminates redundant MOVE instructions after constant
  // loads. Pattern: LOAD_IMM r0, 5; MOVE r1, r0; ... (r0 never used again)
  ...
  for (size_t i = 0; i < func.instructions.size(); i++) {
    ...
    // Mark the destination register as "killed" - it's no longer a constant we
    // can propagate
    if (!instr.operands.empty() &&
        instr.operands[0].type == IRValue::Type::REGISTER) {
      int dst = std::get<int>(instr.operands[0].value);
    ...
    if (instr.opcode == IROpcode::MOVE) {
      ...
      if (constant_regs.count(src)) {
        ...
        new_instructions.emplace_back(info.opcode, IRValue::reg(dst),
                                      info.value);
...
IROptimizer::find_live_registers(const IRFunction &func) {
  ...
  for (const auto &instr : func.instructions) {
    ...
    if (is_branch_op(instr.opcode)) {
      ...
      // Fused branches (EQ/NEQ/LT/etc): first TWO operands are registers to
      // compare
      size_t num_reg_operands = (instr.opcode == IROpcode::BRANCH_ZERO ||
                                 instr.opcode == IROpcode::BRANCH_NOT_ZERO)
                                    ? 1
                                    : 2;
      for (size_t i = 0; i < num_reg_operands && i < instr.operands.size();
           i++) {
...
bool IROptimizer::is_register_used_after(const IRFunction &func, int reg,
                                         size_t instr_idx) {
  for (size_t i = instr_idx; i < func.instructions.size(); i++) {
    ...
    for (size_t j = 0; j < instr.operands.size(); j++) {
      if (instr.operands[j].type == IRValue::Type::REGISTER) {
        ...
        if (r == reg) {
          // For most instructions, first operand is destination, rest are
          // sources Exception: BRANCH_ZERO, BRANCH_NOT_ZERO read first operand
          if (j > 0 || instr.opcode == IROpcode::BRANCH_ZERO ||
              instr.opcode == IROpcode::BRANCH_NOT_ZERO) {
            return true;
          }
        }
...
void IROptimizer::invalidate_register(int reg) { m_constants.erase(reg); }
...
bool IROptimizer::is_reg_used_between_exclusive(const IRFunction &func, int reg,
                                                size_t start_idx,
                                                size_t end_idx,
                                                bool conservative_at_labels) {
  ...
  for (size_t i = start_idx + 1; i < end_idx; i++) {
    ...
    // LABEL is a control flow boundary - be conservative (unless told
    // otherwise)
    if (instr.opcode == IROpcode::LABEL) {
      return conservative_at_labels;
    }

    // Fused branch instructions read both operands 0 and 1 as source operands
    // (not destinations)
...
static void flush_pending(std::vector<IRInstruction> &new_instructions,
  ...
  if (!pending_stores.empty()) {
    ...
    std::vector<std::pair<int, size_t>> sorted(pending_stores.begin(),
                                               pending_stores.end());
...
static bool
reads_pending_store(const IRInstruction &instr,
...
void IROptimizer::eliminate_redundant_stores(IRFunction &func) {
  ...
  //
  // This is done by delaying the emission of store instructions until we're
  // sure they're not dead (overwritten without being read) or redundant (same
  // value).
  ...
  for (size_t i = 0; i < func.instructions.size(); i++) {
    ...
    if (instr.opcode == IROpcode::LABEL || instr.opcode == IROpcode::JUMP ||
        is_branch_op(instr.opcode) || instr.opcode == IROpcode::CALL ||
    ...
    if (is_pure_load_op(instr.opcode) && !instr.operands.empty() &&
      ...
      if (pending_stores.count(dst)) {
        ...
        bool is_identical =
            (prev_instr.opcode == instr.opcode &&
...
std::vector<IROptimizer::LoopInfo>
IROptimizer::identify_loops(const IRFunction &func) {
  ...
  for (size_t i = 0; i < func.instructions.size(); i++) {
    if (func.instructions[i].opcode == IROpcode::LABEL) {
      std::string label =
          std::get<std::string>(func.instructions[i].operands[0].value);
  ...
  std::unordered_map<std::string, std::string>
      loop_to_exit; // header -> exit label
        ...
        for (size_t j = header_idx; j < i; j++) {
          ...
          if (is_branch_op(loop_instr.opcode)) {
            ...
            if (label_idx < loop_instr.operands.size() &&
              ...
              exit_label =
                  std::get<std::string>(loop_instr.operands[label_idx].value);
              break;
            }
          }
        }
...
bool IROptimizer::is_loop_invariant(
    const IRInstruction &instr, const LoopInfo &loop, const IRFunction &func,
    const std::unordered_set<int> &invariant_regs) {
  // Only consider pure operations
  if (!is_pure_load_op(instr.opcode)) {
    return false;
  }

  // LOAD_IMM, LOAD_FLOAT_IMM, LOAD_BOOL are always invariant (no register
  // operands)
  if (instr.opcode == IROpcode::LOAD_IMM ||
      instr.opcode == IROpcode::LOAD_FLOAT_IMM ||
...
bool IROptimizer::can_safely_hoist(const IRInstruction &instr, size_t instr_idx,
                                   const LoopInfo &loop,
                                   const IRFunction &func) {
  // Check that:
  ...
  for (size_t i = loop.header_idx;
       i < loop.end_idx && i < func.instructions.size(); i++) {
    ...
    if (!loop_instr.operands.empty() &&
        loop_instr.operands[0].type == IRValue::Type::REGISTER) {
    ...
    // Check if dst_reg is read before being written (only before this
    // instruction)
    if (i < instr_idx) {
...
void IROptimizer::loop_invariant_code_motion(IRFunction &func) {
  ...
  for (const auto &loop : loops) {
    for (const auto &other : loops) {
      ...
      if (loop.header_idx > other.header_idx &&
  ...
  for (const auto &loop : loops) {
    ...
    while (loop_changed) {
      ...
      for (size_t i = loop.header_idx + 1;
           i < loop.end_idx && i < func.instructions.size(); i++) {
    ...
    if (!invariant_instrs.empty()) {
      ...
      for (size_t i = 0; i < func.instructions.size(); i++) {
        if (i >= loop.header_idx && i < loop.end_idx &&
            invariant_instrs.count(i)) {
      ...
      for (size_t i = 0; i < remaining.size(); i++) {
        if (remaining[i].opcode == IROpcode::LABEL && i < remaining.size() &&
            std::get<std::string>(remaining[i].operands[0].value) ==
                loop.header_label) {
          // Insert hoisted instructions before loop header
          new_instructions.insert(new_instructions.end(), hoisted.begin(),
                                  hoisted.end());
        }
...
bool IROptimizer::register_unmodified_between(const IRFunction &func, int reg,
                                              size_t start_idx,
                                              size_t end_idx) {
  for (size_t i = start_idx + 1; i < end_idx && i < func.instructions.size();
       i++) {
    const auto &instr = func.instructions[i];
...
void IROptimizer::enhanced_copy_propagation(IRFunction &func) {
  ...
  for (size_t i = 0; i < func.instructions.size(); i++) {
    ...
    // Propagate copies in operands (skip first operand which is usually
    // destination)
    bool modified = false;
    for (size_t j = 1; j < instr.operands.size(); j++) {
      if (instr.operands[j].type == IRValue::Type::REGISTER) {
        ...
        if (copies.count(reg)) {
          ...
          if (register_unmodified_between(func, copy_info.source_reg,
                                          copy_info.def_idx, i)) {
