...
static IRInstruction::TypeHint
type_hint_from_string(const std::string &type_str) {
  // Basic types
  if (type_str == "int") {
    return Variant::INT;
...
IRProgram CodeGenerator::generate(const Program &program) {
  ...
  for (size_t i = 0; i < program.globals.size(); i++) {
    ...
    // This is necessary for complex types (String, Array, Dictionary, etc.)
    // which require VASSIGN for proper reference counting. Without type
    // information, we cannot determine at compile time whether VASSIGN is
    // needed.
    if (global.type_hint.empty() && !global.initializer) {
      throw CompilerException(
          ErrorType::CODEGEN_ERROR,
          "Global variable '" + global.name +
              "' requires either a type hint or an initializer. " +
              "Please add ': type' (e.g., ': Array') or an initializer (e.g., "
              "'= []'). " +
              "This is required to ensure proper memory management for complex "
              "types.");
    }
    ...
    if (global.initializer) {
      if (auto *lit =
              dynamic_cast<const LiteralExpr *>(global.initializer.get())) {
        switch (lit->lit_type) {
      ...
      } else if (auto *array_lit = dynamic_cast<const ArrayLiteralExpr *>(
                     global.initializer.get())) {
      ...
      // For other non-literal initializers, we'll need to generate
      // initialization code This would be done in a special initialization
      // function or at first access
    }
...
void CodeGenerator::gen_assign(const AssignStmt *stmt, IRFunction &func) {
  ...
  // Check if this is an indexed assignment (arr[0] = value) or property
  // assignment (obj.prop = value)
  if (stmt->target) {
    // Check for indexed assignment: arr[idx] = value
    if (auto *index_expr =
            dynamic_cast<const IndexExpr *>(stmt->target.get())) {
    ...
    if (auto *member_expr =
            dynamic_cast<const MemberCallExpr *>(stmt->target.get())) {
      ...
      vset_instr.operands.push_back(IRValue::imm(
          static_cast<int64_t>(member_expr->member_name.length())));
  ...
  if (is_global_variable(stmt->name)) {
    ...
    func.instructions.emplace_back(IROpcode::STORE_GLOBAL,
                                   IRValue::imm(global_idx),
  ...
  if (var->is_const) {
    throw CompilerException(ErrorType::CODEGEN_ERROR,
                            "Cannot assign to const variable: " + stmt->name);
  }
...
void CodeGenerator::gen_return(const ReturnStmt *stmt, IRFunction &func) {
  if (stmt->value) {
    ...
    if (reg != 0) {
      func.instructions.emplace_back(IROpcode::MOVE, IRValue::reg(0),
                                     IRValue::reg(reg));
    }
    free_register(reg);
  }
...
void CodeGenerator::gen_if(const IfStmt *stmt, IRFunction &func) {
  ...
  if (!stmt->else_branch.empty()) {
    func.instructions.emplace_back(IROpcode::BRANCH_ZERO,
                                   IRValue::reg(cond_reg),
                                   IRValue::label(else_label));
  } else {
    func.instructions.emplace_back(IROpcode::BRANCH_ZERO,
                                   IRValue::reg(cond_reg),
                                   IRValue::label(end_label));
  }
...
void CodeGenerator::gen_while(const WhileStmt *stmt, IRFunction &func) {
  ...
  // Evaluate condition
  int cond_reg = gen_expr(stmt->condition.get(), func);
  func.instructions.emplace_back(IROpcode::BRANCH_ZERO, IRValue::reg(cond_reg),
                                 IRValue::label(end_label));
  free_register(cond_reg);
...
void CodeGenerator::gen_for(const ForStmt *stmt, IRFunction &func) {
  ...
  if (literal) {
    if (literal->lit_type == LiteralExpr::Type::INTEGER ||
      ...
      throw CompilerException(
          ErrorType::CODEGEN_ERROR,
          "Cannot iterate over non-iterable type in 'for' loop. Did you mean "
          "'for " +
              stmt->variable + " in range(" +
              (literal->lit_type == LiteralExpr::Type::INTEGER
                   ? std::to_string(std::get<int64_t>(literal->value))
                   : "N") +
              "):'?");
    }
  }
  ...
  if (!is_range) {
    ...
    auto &index_load = func.instructions.emplace_back(
        IROpcode::LOAD_IMM, IRValue::reg(index_reg), IRValue::imm(0));
    ...
    auto &cmp_instr = func.instructions.emplace_back(
        IROpcode::CMP_LT, IRValue::reg(cond_reg), IRValue::reg(index_reg),
        IRValue::reg(size_reg));
    ...
    at_syscall.operands.push_back(
        IRValue::reg(elem_reg)); // result register (element)
    ...
    func.instructions.emplace_back(IROpcode::LABEL,
                                   IRValue::label(continue_label));

    // Increment: index = index + 1
    int new_idx_reg = alloc_register();
    auto &add_instr = func.instructions.emplace_back(
        IROpcode::ADD, IRValue::reg(new_idx_reg), IRValue::reg(index_reg),
        IRValue::imm(1));
    auto &move_instr = func.instructions.emplace_back(
        IROpcode::MOVE, IRValue::reg(index_reg), IRValue::reg(new_idx_reg));
    ...
    pop_scope();
    m_loop_stack.pop_back();
    // Note: array_reg and size_reg are allocated inside the loop, so they're
  ...
  if (call_expr->arguments.size() == 1) {
    ...
    auto &start_instr = func.instructions.emplace_back(
        IROpcode::LOAD_IMM, IRValue::reg(start_reg), IRValue::imm(0));
    end_reg = gen_expr(call_expr->arguments[0].get(), func);
    step_reg = alloc_register();
    auto &step_instr = func.instructions.emplace_back(
        IROpcode::LOAD_IMM, IRValue::reg(step_reg), IRValue::imm(1));
  ...
  auto &move_instr = func.instructions.emplace_back(
      IROpcode::MOVE, IRValue::reg(loop_var_reg), IRValue::reg(start_reg));
  ...
  if (step_is_constant) {
    ...
    if (step_value >= 0) {
      ...
      auto &cmp_instr = func.instructions.emplace_back(
          IROpcode::CMP_LT, IRValue::reg(cond_reg), IRValue::reg(loop_var_reg),
          IRValue::reg(end_reg));
      cmp_instr.type_hint = Variant::INT; // range() always produces integers
    } else {
      // Backward iteration: loop_var > end
      auto &cmp_instr = func.instructions.emplace_back(
          IROpcode::CMP_GT, IRValue::reg(cond_reg), IRValue::reg(loop_var_reg),
          IRValue::reg(end_reg));
      cmp_instr.type_hint = Variant::INT; // range() always produces integers
    }
  } else {
    ...
    func.instructions.emplace_back(IROpcode::LOAD_IMM, IRValue::reg(zero_reg),
                                   IRValue::imm(0));

    int step_sign_reg = alloc_register();
    auto &step_cmp = func.instructions.emplace_back(
        IROpcode::CMP_GTE, IRValue::reg(step_sign_reg), IRValue::reg(step_reg),
        IRValue::reg(zero_reg));
    step_cmp.type_hint = Variant::INT; // range() always produces integers
    free_register(zero_reg);

    // If step >= 0, use loop_var < end
    func.instructions.emplace_back(IROpcode::BRANCH_NOT_ZERO,
                                   IRValue::reg(step_sign_reg),
                                   IRValue::label(pos_step_label));

    // Negative step: loop_var > end
    auto &neg_cmp = func.instructions.emplace_back(
        IROpcode::CMP_GT, IRValue::reg(cond_reg), IRValue::reg(loop_var_reg),
        IRValue::reg(end_reg));
    neg_cmp.type_hint = Variant::INT; // range() always produces integers
    func.instructions.emplace_back(IROpcode::JUMP,
                                   IRValue::label(check_cond_label));

    // Positive step: loop_var < end
    func.instructions.emplace_back(IROpcode::LABEL,
                                   IRValue::label(pos_step_label));
    auto &pos_cmp = func.instructions.emplace_back(
        IROpcode::CMP_LT, IRValue::reg(cond_reg), IRValue::reg(loop_var_reg),
        IRValue::reg(end_reg));
  ...
  auto &add_instr = func.instructions.emplace_back(
      IROpcode::ADD, IRValue::reg(new_val_reg), IRValue::reg(loop_var_reg),
      IRValue::reg(step_reg));
  add_instr.type_hint = Variant::INT; // range() always produces integers
  auto &move_instr2 = func.instructions.emplace_back(
      IROpcode::MOVE, IRValue::reg(loop_var_reg), IRValue::reg(new_val_reg));
...
void CodeGenerator::gen_break(const BreakStmt *stmt, IRFunction &func) {
  ...
  func.instructions.emplace_back(
      IROpcode::JUMP, IRValue::label(m_loop_stack.back().break_label));
}
...
void CodeGenerator::gen_continue(const ContinueStmt *stmt, IRFunction &func) {
  ...
  func.instructions.emplace_back(
      IROpcode::JUMP, IRValue::label(m_loop_stack.back().continue_label));
}
...
int CodeGenerator::gen_expr(const Expr *expr, IRFunction &func) {
  ...
  } else if (auto *dict_lit =
                 dynamic_cast<const DictionaryLiteralExpr *>(expr)) {
...
int CodeGenerator::gen_literal(const LiteralExpr *expr, IRFunction &func) {
  ...
  case LiteralExpr::Type::STRING: {
    ...
    IRInstruction instr(IROpcode::LOAD_STRING, IRValue::reg(reg),
                        IRValue::imm(str_idx));
...
int CodeGenerator::gen_variable(const VariableExpr *expr, IRFunction &func) {
  ...
  if (is_global_variable(expr->name)) {
    ...
    func.instructions.emplace_back(IROpcode::LOAD_GLOBAL,
  ...
  func.instructions.emplace_back(IROpcode::MOVE, IRValue::reg(new_reg),
                                 IRValue::reg(var->register_num));
...
int CodeGenerator::gen_binary(const BinaryExpr *expr, IRFunction &func) {
  ...
  bool is_arithmetic =
      (expr->op == BinaryExpr::Op::ADD || expr->op == BinaryExpr::Op::SUB ||
       expr->op == BinaryExpr::Op::MUL || expr->op == BinaryExpr::Op::DIV ||
       expr->op == BinaryExpr::Op::MOD);
  bool is_comparison =
      (expr->op == BinaryExpr::Op::EQ || expr->op == BinaryExpr::Op::NEQ ||
       expr->op == BinaryExpr::Op::LT || expr->op == BinaryExpr::Op::LTE ||
       expr->op == BinaryExpr::Op::GT || expr->op == BinaryExpr::Op::GTE);
  ...
  if (is_arithmetic || is_comparison) {
    if (left_type == Variant::INT && right_type == Variant::INT) {
      ...
      result_type = Variant::FLOAT;
               ...
               left_type == right_type && TypeHintUtils::is_vector(left_type)) {
  ...
  IRInstruction instr(op, IRValue::reg(result_reg), IRValue::reg(left_reg),
...
int CodeGenerator::gen_unary(const UnaryExpr *expr, IRFunction &func) {
  ...
  IROpcode op =
      (expr->op == UnaryExpr::Op::NEG) ? IROpcode::NEG : IROpcode::NOT;
  func.instructions.emplace_back(op, IRValue::reg(result_reg),
                                 IRValue::reg(operand_reg));
...
int CodeGenerator::gen_call(const CallExpr *expr, IRFunction &func) {
  ...
  if (expr->function_name == "get_node") {
    ...
    if (arg_regs.size() > 1) {
      throw CompilerException(ErrorType::CODEGEN_ERROR,
                              "get_node() takes at most 1 argument");
    }
  ...
  if (is_local_function(expr->function_name)) {
    ...
    // Generate CALL instruction with function name, result register, and
    // argument registers Format: CALL function_name, result_reg, arg_count,
    // arg1_reg, arg2_reg, ...
  ...
  // Format: VCALL result_reg, self_reg, method_name, arg_count, arg1_reg,
  // arg2_reg, ...
  IRInstruction vcall_instr(IROpcode::VCALL);
  vcall_instr.operands.push_back(IRValue::reg(result_reg));
...
int CodeGenerator::gen_member_call(const MemberCallExpr *expr,
                                   IRFunction &func) {
  ...
  if (!expr->is_method_call && arg_regs.empty()) {
    ...
    if (is_inline_member_access(obj_type, expr->member_name)) {
      int result =
          gen_inline_member_get(obj_reg, obj_type, expr->member_name, func);
      free_register(obj_reg);
      return result;
    }
    ...
    vget_instr.operands.push_back(
        IRValue::imm(static_cast<int64_t>(expr->member_name.length())));
...
int CodeGenerator::gen_array_literal(const ArrayLiteralExpr *expr,
                                     IRFunction &func) {
  ...
  instr.operands.push_back(
      IRValue::imm(static_cast<int>(elem_regs.size()))); // element count
...
int CodeGenerator::gen_dictionary_literal(const DictionaryLiteralExpr *expr,
                                          IRFunction &func) {
  ...
  instr.operands.push_back(IRValue::reg(result_reg));
  instr.operands.push_back(
      IRValue::imm(static_cast<int>(key_regs.size()))); // pair count
...
int CodeGenerator::alloc_register() { return m_next_register++; }
...
void CodeGenerator::pop_scope() {
  if (m_scope_stack.empty()) {
    throw CompilerException(ErrorType::CODEGEN_ERROR,
                            "Cannot pop scope: scope stack is empty");
  }
...
void CodeGenerator::declare_variable(const std::string &name, int register_num,
  ...
  if (m_scope_stack.empty()) {
    throw CompilerException(ErrorType::CODEGEN_ERROR,
                            "Cannot declare variable: no scope active");
  }

  // Check if variable already exists in current scope (shadowing is allowed,
  // but redeclaration in same scope is not)
  ...
  current_scope.variables[name] = {name, register_num,
                                   IRInstruction::TypeHint_NONE, is_const};
}
...
bool CodeGenerator::is_inline_primitive_constructor(
    const std::string &name) const {
  return name == "Vector2" || name == "Vector3" || name == "Vector4" ||
         name == "Vector2i" || name == "Vector3i" || name == "Vector4i" ||
         name == "Color" || name == "Rect2" || name == "Rect2i" ||
         name == "Plane" || name == "Array" || name == "Dictionary" ||
...
bool CodeGenerator::is_inline_member_access(IRInstruction::TypeHint type,
...
int CodeGenerator::gen_inline_constructor(const std::string &name,
  ...
  if (name == "Vector2" && arg_regs.size() == 2) {
    ...
    if (arg_regs.size() == 0) {
      ...
      func.instructions.emplace_back(IROpcode::LOAD_FLOAT_IMM,
                                     IRValue::reg(r_reg), IRValue::fimm(1.0));
      func.instructions.emplace_back(IROpcode::LOAD_FLOAT_IMM,
                                     IRValue::reg(g_reg), IRValue::fimm(1.0));
      func.instructions.emplace_back(IROpcode::LOAD_FLOAT_IMM,
                                     IRValue::reg(b_reg), IRValue::fimm(1.0));
      func.instructions.emplace_back(IROpcode::LOAD_FLOAT_IMM,
                                     IRValue::reg(a_reg), IRValue::fimm(1.0));
      ...
      throw CompilerException(
          ErrorType::CODEGEN_ERROR,
          "Color constructor requires 0, 3, or 4 arguments");
    }
  } else if (name == "Array") {
    ...
    instr.operands.push_back(IRValue::reg(result_reg));
    instr.operands.push_back(
        IRValue::imm(static_cast<int>(arg_regs.size()))); // element count
    for (int arg_reg : arg_regs) {
...
int CodeGenerator::gen_inline_member_get(int obj_reg,
                                         IRInstruction::TypeHint obj_type,
  ...
  bool is_int_vector =
      (obj_type == Variant::VECTOR2I || obj_type == Variant::VECTOR3I ||
...
int CodeGenerator::gen_global_class_get(const std::string &class_name,
                                        IRFunction &func) {
  // Generate a CALL_SYSCALL instruction to get the global class object
  // ECALL_GET_OBJ (504) takes: a0 = result pointer, a1 = class name pointer, a2
  // = class name length Returns: a0 contains the object data
  ...
  // Format: CALL_SYSCALL result_reg, syscall_number, string_index,
  // string_length
  IRInstruction instr(IROpcode::CALL_SYSCALL);
  instr.operands.push_back(IRValue::reg(result_reg)); // result register
  ...
  instr.operands.push_back(
      IRValue::imm(static_cast<int64_t>(class_name.length()))); // string length

  func.instructions.push_back(instr);

  // The result is an OBJECT Variant
  set_register_type(result_reg,
                    IRInstruction::TypeHint_NONE); // Objects don't have a
