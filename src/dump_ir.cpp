  ...
  try {
    ...
    if (verbose) {
      ...
      for (const auto &tok : tokens) {
        std::cout << "  " << tok.lexeme
                  << " [type=" << token_type_name(tok.type) << "]" << std::endl;
      }
      std::cout << std::endl;
    }
    ...
    if (!ir.string_constants.empty()) {
      ...
      for (size_t i = 0; i < ir.string_constants.size(); i++) {
        std::cout << "  [" << i << "] \"" << ir.string_constants[i] << "\""
                  << std::endl;
      }
      std::cout << std::endl;
    }
    ...
    for (const auto &func : ir.functions) {
      ...
      enum class ValueType { UNKNOWN, VARIANT };
      ...
      for (size_t i = 0; i < func.instructions.size(); i++) {
        ...
        for (size_t j = 0; j < instr.operands.size(); j++) {
          ...
          if (show_codegen &&
              instr.operands[j].type == IRValue::Type::REGISTER) {
            ...
            if (vreg_types.count(vreg)) {
              ...
              if (vreg_types[vreg] == ValueType::VARIANT &&
                  variant_offsets.count(vreg)) {
