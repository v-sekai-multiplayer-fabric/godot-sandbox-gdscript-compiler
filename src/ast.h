...
struct BinaryExpr : Expr {
  enum class Op { ADD, SUB, MUL, DIV, MOD, EQ, NEQ, LT, LTE, GT, GTE, AND, OR };
...
struct MemberCallExpr : Expr {
  ...
  bool is_method_call =
      false; // true if this is obj.method(), false if obj.property

  MemberCallExpr(ExprPtr obj, std::string name, std::vector<ExprPtr> args = {},
                 bool is_method = false)
      : object(std::move(obj)), member_name(std::move(name)),
        arguments(std::move(args)), is_method_call(is_method) {}
};
...
struct VarDeclStmt : Stmt {
  std::string name;
  std::string
      type_hint; // Type annotation if present (e.g., "int", "float", "String")
...
struct IfStmt : Stmt {
  ...
  IfStmt(ExprPtr cond, std::vector<StmtPtr> then_b,
...
struct FunctionDecl {
  ...
  std::string
      return_type; // Return type annotation if present (e.g., "void", "int")
  std::vector<StmtPtr> body;
  int line = 0;
  int column = 0;
};
