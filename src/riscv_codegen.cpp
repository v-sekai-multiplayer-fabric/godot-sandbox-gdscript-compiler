...
void RISCVCodeGen::emit_variant_component_to_float(int comp_offset,
                                                   int result_offset,
                                                   int store_offset,
                                                   bool normalize_by_255) {
  // Convert a Variant component (at comp_offset) to float and store to
  // result_offset + store_offset Handles both INT (type=2) and FLOAT (type=3)
  // Variants If normalize_by_255 is true, divides INTEGER values by 255.0 (for
  // Color components)
  ...
  // Note: We do NOT normalize float values - they're already in the correct
  // range
  emit_fcvt_s_d(REG_FA0, REG_FA0); // Convert double to float
...
std::vector<uint8_t> RISCVCodeGen::generate(const IRProgram &program) {
  ...
  for (size_t i = 0; i < program.globals.size(); i++) {
    ...
    if (global.init_type == IRGlobalVar::InitType::INT) {
      ...
      int str_space = ((str_len + 1) + 7) &
                      ~7;    // String + null terminator, aligned to 8 bytes
      ...
      if (str_space < 2048) {
        emit_i_type(0x13, REG_T2, 0, REG_SP,
                    0); // T2 = SP + 0 (pointer to string)
      } else {
      ...
      // T0 still points to the global Variant (loaded with la before stack
      // adjustment) We need the offset from current SP to pass to
      // emit_vcreate_syscall Since T0 is an absolute address, we can't use it
      // directly with emit_vcreate_syscall We need to use VCREATE directly here
      ...
      if (global.type_hint != IRInstruction::TypeHint_NONE) {
        ...
        default:
          throw CompilerException(
              ErrorType::RISCV_codegen_ERROR,
              "Global variable '" + global.name +
                  "': Could not derive Variant type from initializer.");
        }
      }
  ...
  // Calculate global data size (m_global_count and m_globals already set
  // earlier)
  ...
  if (m_global_count > 0) {
    ...
    // The .data segment will be loaded at: BASE_ADDR + text_size, aligned to
    // 4KB
    ...
    // Since BASE_ADDR (0x10000) is added during resolution, we store: vaddr -
    // BASE_ADDR
...
void RISCVCodeGen::gen_function(const IRFunction &func) {
  ...
  // cause different virtual registers to get different offsets, breaking the
  // code.
  ...
  for (size_t i = 0; i < m_num_params && i < 7; i++) {
    int param_vreg =
        static_cast<int>(i); // Parameters map to virtual registers 0-6
  ...
  for (const auto &instr : func.instructions) {
    ...
    case IROpcode::LOAD_GLOBAL: {
      ...
      // Loads a global variable (Variant) from the global data area into a
      // virtual register
      int dst_vreg = std::get<int>(instr.operands[0].value);
    ...
    case IROpcode::STORE_GLOBAL: {
      ...
      if (needs_vassign) {
        ...
        auto moves = m_allocator.handle_syscall_clobbering(clobbered_regs,
                                                           m_current_instr_idx);
    ...
    case IROpcode::MOD: {
      ...
      if (instr.operands.size() < 3 ||
        ...
        throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                                "Arithmetic operations require at least 3 "
                                "operands with first being REGISTER");
      }
      ...
      bool rhs_is_reg = instr.operands.size() > 2 &&
      ...
      if (instr.type_hint != IRInstruction::TypeHint_NONE && lhs_is_reg &&
          rhs_is_reg) {
    ...
    case IROpcode::NEG: {
      ...
      // Actually for unary, we need a different approach - create a zero
      // Variant For now, use subtract: 0 - src
    ...
    case IROpcode::CMP_GTE: {
      ...
      if (instr.operands.size() < 3 ||
          instr.operands[0].type != IRValue::Type::REGISTER) {
        throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                                "Comparison operations require at least 3 "
                                "operands with first being REGISTER");
      }
      ...
      if (instr.type_hint == Variant::INT && lhs_is_reg && rhs_is_reg) {
        ...
        emit_typed_int_comparison(dst_offset, lhs_offset, rhs_offset,
                                  instr.opcode);
        break;
      }

      // Fall back to generic Variant evaluation for untyped or non-INT
      // comparisons
    ...
    case IROpcode::BRANCH_ZERO: {
      ...
      mark_label_use(std::get<std::string>(instr.operands[1].value),
                     m_code.size());
    ...
    case IROpcode::BRANCH_GTE: {
      ...
      if (instr.operands.size() < 3) {
        throw CompilerException(
            ErrorType::RISCV_codegen_ERROR,
            "Fused branch requires 3 operands: lhs, rhs, label");
      }
      ...
      // For now, only support register operands (can be extended later for
      // immediates)
      if (!lhs_is_reg || !rhs_is_reg) {
        throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                                "Fused branch requires register operands");
      }
    ...
    case IROpcode::JUMP:
      mark_label_use(std::get<std::string>(instr.operands[0].value),
                     m_code.size());
      emit_jal(REG_ZERO, 0);
      break;
    ...
    case IROpcode::VCALL: {
      // VCALL format: result_reg, obj_reg, method_name, arg_count, arg1_reg,
      // arg2_reg, ...
      if (instr.operands.size() < 4) {
        throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                                "VCALL requires at least 4 operands");
      }
      ...
      int arg_count =
          static_cast<int>(std::get<int64_t>(instr.operands[3].value));

      if (instr.operands.size() != static_cast<size_t>(4 + arg_count)) {
        throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                                "VCALL argument count mismatch");
      }
      ...
      if (arg_count > 0) {
        ...
        for (int i = 0; i < arg_count; i++) {
          ...
          int arg_src_offset = get_variant_stack_offset(arg_vreg) +
                               additional_space; // Adjust for moved stack
      ...
      // a0 = pointer to object Variant (sp + obj_offset + additional_space if
      // we allocated stack)
      ...
      int str_space = ((method_len + 1) + 7) &
                      ~7; // Align to 8 bytes, +1 for null terminator
      ...
      // VCALL always writes the result as a full Variant on stack (via a5
      // pointer)
      break;
    }
    ...
    case IROpcode::CALL: {
      ...
      std::vector<uint8_t> clobbered_regs = {REG_A0, REG_A1, REG_A2,
                                             REG_A3, REG_A4, REG_A5,
                                             REG_A6, REG_A7, REG_RA};
      auto moves = m_allocator.handle_syscall_clobbering(clobbered_regs,
                                                         m_current_instr_idx);
    ...
    case IROpcode::MAKE_VECTOR4: {
      ...
      int num_components = (instr.opcode == IROpcode::MAKE_VECTOR2)   ? 2
                           : (instr.opcode == IROpcode::MAKE_VECTOR3) ? 3
                                                                      : 4;

      if (instr.operands.size() != static_cast<size_t>(1 + num_components)) {
        throw CompilerException(
            ErrorType::RISCV_codegen_ERROR,
            "MAKE_VECTOR requires correct number of operands");
      }
      ...
      int variant_type =
          (instr.opcode == IROpcode::MAKE_VECTOR2)   ? Variant::VECTOR2
      ...
      // Store each component at offset 8, 12, 16, 20 (4 bytes per component as
      // real_t=float)
    ...
    case IROpcode::MAKE_VECTOR4I: {
      ...
      int num_components = (instr.opcode == IROpcode::MAKE_VECTOR2I)   ? 2
                           : (instr.opcode == IROpcode::MAKE_VECTOR3I) ? 3
                                                                       : 4;

      if (instr.operands.size() != static_cast<size_t>(1 + num_components)) {
        throw CompilerException(
            ErrorType::RISCV_codegen_ERROR,
            "MAKE_VECTORnI requires correct number of operands");
      }
      ...
      // Set m_type field (offset 0)
      int variant_type =
          (instr.opcode == IROpcode::MAKE_VECTOR2I)   ? Variant::VECTOR2I
    ...
    case IROpcode::MAKE_COLOR: {
      ...
      if (instr.operands.size() != 5) {
        throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                                "MAKE_COLOR requires 5 operands");
      }
      ...
      // Color components need to be normalized (0-255 integers -> 0.0-1.0
      // floats)
      for (int i = 0; i < 4; i++) {
        int comp_vreg = std::get<int>(instr.operands[1 + i].value);
        int comp_offset = get_variant_stack_offset(comp_vreg);
        emit_variant_component_to_float(comp_offset, result_offset, 8 + i * 4,
                                        true); // normalize_by_255 = true
      }
    ...
    case IROpcode::MAKE_ARRAY: {
      // Format: MAKE_ARRAY result_reg, element_count, [element_reg1,
      // element_reg2, ...] For empty arrays: element_count = 0, no element regs
      if (instr.operands.size() < 2) {
        throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                                "MAKE_ARRAY requires at least 2 operands");
      }

      int result_vreg = std::get<int>(instr.operands[0].value);
      int element_count =
          static_cast<int>(std::get<int64_t>(instr.operands[1].value));
      int result_offset = get_variant_stack_offset(result_vreg);
      ...
      if (element_count == 0) {
        ...
        // GuestVariant is 24 bytes (4-byte type + 4-byte padding + 16-byte
        // data) We need to copy the full Variant structures to stack
        ...
        for (int i = 0; i < element_count; i++) {
          ...
          // After SP -= args_space, it's now at elem_offset + args_space from
          // NEW SP So we load from (elem_offset + args_space) and store to
          // dst_offset
        ...
        // After SP -= args_space, it's now at result_offset + args_space from
        // NEW SP
    ...
    case IROpcode::MAKE_DICTIONARY: {
      // Format: MAKE_DICTIONARY result_reg, pair_count, [key1_reg, val1_reg,
      // key2_reg, val2_reg, ...] For empty dictionaries via Dictionary()
      // constructor: MAKE_DICTIONARY result_reg For empty dictionaries via
      // literal {}: MAKE_DICTIONARY result_reg, 0 For dictionaries with
      // elements: pair_count > 0 with key/value regs
      if (instr.operands.size() < 1) {
        throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                                "MAKE_DICTIONARY requires at least 1 operand");
      }

      int result_vreg = std::get<int>(instr.operands[0].value);
      int result_offset = get_variant_stack_offset(result_vreg);

      // Check if this is the old Dictionary() constructor format (only 1
      // operand) or the new dictionary literal format (2+ operands)
      bool is_constructor_format = (instr.operands.size() == 1);
      int pair_count =
          is_constructor_format
              ? 0
              : static_cast<int>(std::get<int64_t>(instr.operands[1].value));
      ...
      if (pair_count == 0) {
        ...
        // Dictionary with elements: sys_vcreate(&v, DICTIONARY, size,
        // data_pointer) GuestVariant is 24 bytes (4-byte type + 4-byte padding
        // + 16-byte data) We need to copy the full Variant structures to stack
        constexpr int VARIANT_SIZE = 24;
        int total_variants =
            pair_count * 2; // Each pair has 2 variants (key and value)
        ...
        for (int i = 0; i < total_variants; i++) {
          ...
          // The variant Variant is at variant_offset from the ORIGINAL stack
          // frame After SP -= args_space, it's now at variant_offset +
          // args_space from NEW SP So we load from (variant_offset +
          // args_space) and store to dst_offset
    ...
    case IROpcode::MAKE_PACKED_VECTOR4_ARRAY: {
      // Format: MAKE_PACKED_*_ARRAY result_reg, element_count, [element_reg1,
      // element_reg2, ...] Works identically to MAKE_ARRAY but with different
      // Variant type
      if (instr.operands.size() < 2) {
        throw CompilerException(
            ErrorType::RISCV_codegen_ERROR,
            "Packed array constructor requires at least 2 operands");
      }
      ...
      if (element_count == 0) {
        ...
        // a0 = op (the packed array type, which also indicates
        // CREATE_FROM_ARRAY)
    ...
    case IROpcode::VGET_INLINE: {
      ...
      if (instr.operands.size() != 4) {
        throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                                "VGET_INLINE requires 4 operands");
      }
      ...
      // Vector types with float components (real_t=float=4 bytes): offsets at
      // 8, 12, 16, 20 Vector types with int components (int32_t=4 bytes):
      // offsets at 8, 12, 16, 20
      ...
      IRInstruction::TypeHint hint =
          static_cast<IRInstruction::TypeHint>(obj_type_hint);
    ...
    case IROpcode::VGET: {
      ...
      if (instr.operands.size() != 4) {
        throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                                "VGET requires 4 operands (result_reg, "
                                "obj_reg, string_idx, string_len)");
      }
      ...
      if (string_idx < 0 ||
          static_cast<size_t>(string_idx) >= m_string_constants->size()) {
        throw CompilerException(ErrorType::RISCV_codegen_ERROR,
      ...
      // IMPORTANT: Load this BEFORE adjusting the stack, so obj_offset is
      // correct
      emit_ld(REG_A0, REG_SP, obj_offset + 8);

      // Allocate stack space for the string
      int str_space = ((string_len + 1) + 7) &
                      ~7; // Align to 8 bytes, +1 for null terminator
    ...
    case IROpcode::VSET: {
      ...
      if (instr.operands.size() != 4) {
        throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                                "VSET requires 4 operands (obj_reg, "
                                "string_idx, string_len, value_reg)");
      }
      ...
      // A3 = pointer to value Variant
      // IMPORTANT: Calculate this AFTER adjusting the stack, and account for
      // str_space emit_load_stack_offset does: addi a3, sp, offset, so we get
      // the correct address directly
    ...
    case IROpcode::VSET_INLINE:
      throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                              "Opcode not yet implemented in RISC-V codegen");
    ...
    case IROpcode::CALL_SYSCALL: {
      ...
      if (instr.operands.size() < 2) {
        throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                                "CALL_SYSCALL requires at least 2 operands "
                                "(result_reg, syscall_num)");
      }
      ...
      if (syscall_num == 504) {
        ...
        if (instr.operands.size() != 4) {
          throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                                  "ECALL_GET_OBJ requires 4 operands");
        }
        ...
        emit_syscall_result(result_vreg, REG_A0, result_offset,
                            24); // GDOBJECT type = 24
        ...
        if (instr.operands.size() != 3) {
          throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                                  "ECALL_ARRAY_SIZE requires 3 operands");
        }

        int array_vreg =
            static_cast<int>(std::get<int>(instr.operands[2].value));
        ...
        // Takes: a0 = array variant index (unsigned), a1 = index (int), a2 =
        // result GuestVariant pointer Returns: result variant is filled with
        // the element
        if (instr.operands.size() != 4) {
          throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                                  "ECALL_ARRAY_AT requires 4 operands");
        }

        int array_vreg =
            static_cast<int>(std::get<int>(instr.operands[2].value));
        int index_vreg =
            static_cast<int>(std::get<int>(instr.operands[3].value));
        ...
        // The index is stored as a Variant, we need to extract the integer
        // value NOTE: Use emit_ld (64-bit) because integer Variants store
        // 64-bit values, not 32-bit!
        emit_ld(REG_A1, REG_SP,
                index_offset + 8); // Load 64-bit integer from offset 8

        // a2 = pointer to result GuestVariant
        emit_i_type(0x13, REG_A2, 0, REG_SP,
                    result_offset); // addi a2, sp, result_offset
        ...
        // Takes: a0 = base node address (0 for owner), a1 = path string
        // pointer, a2 = path length Returns: a0 = node object reference
        if (instr.operands.size() < 3) {
          throw CompilerException(
              ErrorType::RISCV_codegen_ERROR,
              "ECALL_GET_NODE requires at least 3 operands");
        }
...
void RISCVCodeGen::emit_r_type(uint8_t opcode, uint8_t rd, uint8_t funct3,
                               uint8_t rs1, uint8_t rs2, uint8_t funct7) {
  uint32_t instr = opcode | (rd << 7) | (funct3 << 12) | (rs1 << 15) |
                   (rs2 << 20) | (funct7 << 25);
  emit_word(instr);
}

void RISCVCodeGen::emit_i_type(uint8_t opcode, uint8_t rd, uint8_t funct3,
...
void RISCVCodeGen::emit_s_type(uint8_t opcode, uint8_t funct3, uint8_t rs1,
                               uint8_t rs2, int32_t imm) {
  uint32_t imm_lo = imm & 0x1F;
  uint32_t imm_hi = (imm >> 5) & 0x7F;
  uint32_t instr = opcode | (imm_lo << 7) | (funct3 << 12) | (rs1 << 15) |
                   (rs2 << 20) | (imm_hi << 25);
...
void RISCVCodeGen::emit_b_type(uint8_t opcode, uint8_t funct3, uint8_t rs1,
                               uint8_t rs2, int32_t imm) {
  ...
  uint32_t instr = opcode | (imm11 << 7) | (imm4_1 << 8) | (funct3 << 12) |
...
void RISCVCodeGen::emit_j_type(uint8_t opcode, uint8_t rd, int32_t imm) {
  ...
  uint32_t instr = opcode | (rd << 7) | (imm19_12 << 12) | (imm11 << 20) |
                   (imm10_1 << 21) | (imm20 << 31);
  m_code.push_back(instr & 0xFF);
  m_code.push_back((instr >> 8) & 0xFF);
  m_code.push_back((instr >> 16) & 0xFF);
...
void RISCVCodeGen::emit_r4_type(uint8_t opcode, uint8_t rd, uint8_t funct3,
                                uint8_t rs1, uint8_t rs2, uint8_t rs3,
                                uint8_t funct2) {
  union {
    struct {
      uint32_t opcode : 7;
...
void RISCVCodeGen::mark_label_use(const std::string &label,
                                  size_t code_offset) {
...
void RISCVCodeGen::resolve_labels() {
  for (const auto &use : m_label_uses) {
    ...
    if (opcode == 0x63) { // B-type (branch)
      ...
      instr = opcode | (imm11 << 7) | (imm4_1 << 8) | (funct3 << 12) |
              (rs1 << 15) | (rs2 << 20) | (imm10_5 << 25) | (imm12 << 31);
      ...
      instr = opcode | (rd << 7) | (imm19_12 << 12) | (imm11 << 20) |
              (imm10_1 << 21) | (imm20 << 31);
    } else if (opcode ==
               0x17) { // U-type (AUIPC) - for constant pool or address loading
      uint8_t rd = (instr >> 7) & 0x1F;

      // Check if the next instruction is LD (for constant pool) or ADDI (for
      // address)
      uint32_t next_instr;
      ...
      if (next_opcode == 0x03) { // LD instruction (for constant pool access)
        ...
        next_instr = 0x03 | (ld_rd << 7) | (ld_funct3 << 12) | (ld_rs1 << 15) |
                     ((lower & 0xFFF) << 20);
        ...
        if (addi_rs1 == rd && addi_funct3 == 0) {
          ...
          next_instr = 0x13 | (addi_rd << 7) | (addi_funct3 << 12) |
                       (addi_rs1 << 15) | ((lower & 0xFFF) << 20);
          memcpy(&m_code[use_offset + 4], &next_instr, 4);
...
void RISCVCodeGen::emit_variant_create_int(int stack_offset, int64_t value) {
  ...
  // Variant layout: [uint32_t m_type (4 bytes)] [padding (4 bytes)] [int64_t
  // v.i (8 bytes)]
...
void RISCVCodeGen::emit_variant_create_string(int stack_offset,
                                              int string_idx) {
  // Create a string Variant using VCREATE syscall
  // VCREATE signature: void vcreate(Variant* dst, Variant::Type type, int
  // method, void* data) For strings with method==1: data = struct { const char*
  // str; size_t length; }

  if (!m_string_constants || string_idx < 0 ||
      string_idx >= static_cast<int>(m_string_constants->size())) {
    throw CompilerException(ErrorType::RISCV_codegen_ERROR,
  ...
  // Allocate stack space for: string data + struct { char*, size_t }
  int str_space =
      ((str_len + 1) + 7) & ~7; // String + null terminator, aligned to 8 bytes
  int struct_space = 16;        // Two 8-byte fields
  int total_space = str_space + struct_space;
  total_space = (total_space + 15) & ~15; // Align to 16 bytes
...
void RISCVCodeGen::emit_vcreate_syscall(int variant_type, int method,
                                        uint8_t data_ptr_reg,
                                        int result_offset) {
  // Generic VCREATE syscall emission
  // VCREATE signature: void vcreate(Variant* dst, Variant::Type type, int
  // method, void* data) This handles register clobbering for a0-a3
...
void RISCVCodeGen::emit_variant_eval(int result_offset, int lhs_offset,
                                     int rhs_offset, int op) {
  // Call sys_veval(op, &lhs, &rhs, &result)
  // Signature: bool sys_veval(int op, const Variant* a, const Variant* b,
  // Variant* result)
...
void RISCVCodeGen::emit_typed_int_binary_op(int result_offset, int lhs_offset,
                                            int rhs_offset, IROpcode op) {
  ...
  default:
    throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                            "Unsupported typed int binary op");
  }
...
void RISCVCodeGen::emit_typed_int_comparison(int result_offset, int lhs_offset,
                                             int rhs_offset, IROpcode cmp_op) {
  ...
  default:
    throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                            "Unsupported typed int comparison");
  }
...
void RISCVCodeGen::emit_typed_float_binary_op(int result_offset, int lhs_offset,
                                              int rhs_offset, IROpcode op) {
  ...
  default:
    throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                            "Unsupported typed float binary op");
  }
...
void RISCVCodeGen::emit_typed_vector_binary_op(
    int result_offset, int lhs_offset, int rhs_offset, IROpcode op,
  ...
  for (int i = 0; i < elem_count; i++) {
    ...
    if (is_int) {
      ...
      default:
        throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                                "Unsupported vector int operation");
      }
      ...
      default:
        throw CompilerException(ErrorType::RISCV_codegen_ERROR,
                                "Unsupported vector float operation");
      }
...
void RISCVCodeGen::emit_load_variant_type(uint8_t rd, uint8_t base_reg,
                                          int32_t variant_offset) {
  // Load m_type field (4 bytes at variant_offset + 0)
  emit_lw(rd, base_reg, variant_offset + VARIANT_TYPE_OFFSET);
}

void RISCVCodeGen::emit_store_variant_type(uint8_t rs, uint8_t base_reg,
                                           int32_t variant_offset) {
  // Store m_type field (4 bytes at variant_offset + 0)
  emit_sw(rs, base_reg, variant_offset + VARIANT_TYPE_OFFSET);
}

void RISCVCodeGen::emit_load_variant_bool(uint8_t rd, uint8_t base_reg,
                                          int32_t variant_offset) {
...
void RISCVCodeGen::emit_store_variant_bool(uint8_t rs, uint8_t base_reg,
                                           int32_t variant_offset) {
  // Store boolean value (1 byte at variant_offset + 8)
  // Note: storing 8 bytes is fine too since bool is in a union, but byte is
  // more precise
  emit_sb(rs, base_reg, variant_offset + VARIANT_DATA_OFFSET);
}

void RISCVCodeGen::emit_load_variant_int(uint8_t rd, uint8_t base_reg,
                                         int32_t variant_offset) {
  // Load int64 value (8 bytes at variant_offset + 8)
  emit_ld(rd, base_reg, variant_offset + VARIANT_DATA_OFFSET);
}

void RISCVCodeGen::emit_store_variant_int(uint8_t rs, uint8_t base_reg,
                                          int32_t variant_offset) {
...
void RISCVCodeGen::emit_syscall_result(int result_vreg, uint8_t result_reg,
                                       int result_offset, int variant_type) {
  // Always store to stack as Variant
  emit_li(REG_T0, variant_type);
  emit_sw(REG_T0, REG_SP, result_offset); // Store 4-byte type
  emit_sd(result_reg, REG_SP,
          result_offset + 8); // Store 8-byte integer at offset 8
}
...
bool RISCVCodeGen::is_complex_variant_type(int variant_type) {
  ...
  // Everything else (String, Array, Dictionary, Object, packed arrays, etc.)
