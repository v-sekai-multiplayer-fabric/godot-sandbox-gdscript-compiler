...
RegisterAllocator::RegisterAllocator() { init_free_registers(); }

void RegisterAllocator::init_free_registers() {
  m_free_registers = {REG_T0,  REG_T1,  REG_T2, REG_S1, REG_S2, REG_S3,
                      REG_S4,  REG_S5,  REG_S6, REG_S7, REG_S8, REG_S9,
                      REG_S10, REG_S11, REG_T3, REG_T4, REG_T5, REG_T6};
}
...
RegisterAllocator::handle_syscall_clobbering(
    const std::vector<uint8_t> &clobbered_regs, int current_instr_idx) {
  std::vector<std::pair<uint8_t, uint8_t>> moves;

  for (uint8_t clobbered_preg : clobbered_regs) {
    std::unordered_map<uint8_t, int>::iterator it =
        m_preg_to_vreg.find(clobbered_preg);
...
void RegisterAllocator::invalidate_register(int vreg) {
  // Same as free_register - removes the vreg->preg mapping and frees the
  // physical register This is necessary when the value is updated on the stack
  // through variant operations, to prevent stale physical register values from
  // being used
  std::unordered_map<int, uint8_t>::iterator it = m_vreg_to_preg.find(vreg);
  if (it != m_vreg_to_preg.end()) {
    uint8_t preg = it->second;
...
void RegisterAllocator::force_register_mapping(int vreg, uint8_t preg) {
  ...
  std::unordered_map<int, uint8_t>::iterator vreg_it =
      m_vreg_to_preg.find(vreg);
  ...
  std::vector<uint8_t>::iterator free_it =
      std::find(m_free_registers.begin(), m_free_registers.end(), preg);
...
int RegisterAllocator::get_vreg_for_preg(uint8_t preg) const {
  ...
  std::unordered_map<uint8_t, int>::const_iterator it =
      m_preg_to_vreg.find(preg);
  if (it != m_preg_to_vreg.end()) {
    return it->second;
  }
  return -1;
...
bool RegisterAllocator::is_register_available(uint8_t preg) const {
  return std::find(m_free_registers.begin(), m_free_registers.end(), preg) !=
         m_free_registers.end();
}

int RegisterAllocator::get_next_use(int vreg, int current_instr_idx) const {
  std::unordered_map<int, std::vector<int>>::const_iterator uses_it =
      m_vreg_all_uses.find(vreg);
  ...
  std::vector<int>::const_iterator it =
      std::lower_bound(uses.begin(), uses.end(), current_instr_idx + 1);
