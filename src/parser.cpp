    ...
    if (check(TokenType::EXTENDS)) {
      ...
      if (check(TokenType::VAR)) {
        ...
        if (auto *decl = dynamic_cast<VarDeclStmt *>(var_decl.get())) {
          VarDeclStmt global_decl(decl->name, std::move(decl->initializer),
                                  decl->is_const);
...
StmtPtr Parser::parse_var_decl(bool is_const) {
  ...
  auto stmt = std::make_unique<VarDeclStmt>(name.lexeme, std::move(initializer),
                                            is_const);
...
StmtPtr Parser::parse_for_stmt() {
  ...
  return std::make_unique<ForStmt>(var_name.lexeme, std::move(iterable),
                                   std::move(body));
}
...
StmtPtr Parser::parse_expr_or_assign_stmt() {
  ...
  if (match(TokenType::ASSIGN)) {
    ...
    if (auto *var_expr = dynamic_cast<VariableExpr *>(lhs.get())) {
      ...
      if (member_expr->is_method_call) {
        throw CompilerException::parser_error("Cannot assign to method call",
                                              lhs->line, lhs->column);
      }
      return std::make_unique<AssignStmt>(std::move(lhs), std::move(value));
    } else {
      throw CompilerException::parser_error("Invalid assignment target",
                                            lhs->line, lhs->column);
    }
  }
  ...
  if (auto *var_expr = dynamic_cast<VariableExpr *>(lhs.get())) {
    ...
    if (match(TokenType::PLUS_ASSIGN)) {
      ...
      ExprPtr add_expr = std::make_unique<BinaryExpr>(
          std::move(var_ref), BinaryExpr::Op::ADD, std::move(rhs));
      consume(TokenType::NEWLINE, "Expected newline after assignment");
    ...
    if (match(TokenType::MINUS_ASSIGN)) {
      ExprPtr var_ref = std::make_unique<VariableExpr>(name);
      ExprPtr rhs = parse_expression();
      ExprPtr sub_expr = std::make_unique<BinaryExpr>(
          std::move(var_ref), BinaryExpr::Op::SUB, std::move(rhs));
      consume(TokenType::NEWLINE, "Expected newline after assignment");
    ...
    if (match(TokenType::MULTIPLY_ASSIGN)) {
      ExprPtr var_ref = std::make_unique<VariableExpr>(name);
      ExprPtr rhs = parse_expression();
      ExprPtr mul_expr = std::make_unique<BinaryExpr>(
          std::move(var_ref), BinaryExpr::Op::MUL, std::move(rhs));
      consume(TokenType::NEWLINE, "Expected newline after assignment");
    ...
    if (match(TokenType::DIVIDE_ASSIGN)) {
      ExprPtr var_ref = std::make_unique<VariableExpr>(name);
      ExprPtr rhs = parse_expression();
      ExprPtr div_expr = std::make_unique<BinaryExpr>(
          std::move(var_ref), BinaryExpr::Op::DIV, std::move(rhs));
      consume(TokenType::NEWLINE, "Expected newline after assignment");
    ...
    if (match(TokenType::MODULO_ASSIGN)) {
      ExprPtr var_ref = std::make_unique<VariableExpr>(name);
      ExprPtr rhs = parse_expression();
      ExprPtr mod_expr = std::make_unique<BinaryExpr>(
          std::move(var_ref), BinaryExpr::Op::MOD, std::move(rhs));
      consume(TokenType::NEWLINE, "Expected newline after assignment");
...
ExprPtr Parser::parse_or_expression() {
  ...
  while (match(TokenType::OR)) {
    ...
    left = std::make_unique<BinaryExpr>(std::move(left), BinaryExpr::Op::OR,
                                        std::move(right));
  }
...
ExprPtr Parser::parse_and_expression() {
  ...
  while (match(TokenType::AND)) {
    ...
    left = std::make_unique<BinaryExpr>(std::move(left), BinaryExpr::Op::AND,
                                        std::move(right));
  }
...
ExprPtr Parser::parse_equality() {
  ...
  while (match_one_of({TokenType::EQUAL, TokenType::NOT_EQUAL})) {
    ...
    BinaryExpr::Op bin_op = (op.type == TokenType::EQUAL) ? BinaryExpr::Op::EQ
                                                          : BinaryExpr::Op::NEQ;
    left =
        std::make_unique<BinaryExpr>(std::move(left), bin_op, std::move(right));
  }
...
ExprPtr Parser::parse_comparison() {
  ...
  while (match_one_of({TokenType::LESS, TokenType::LESS_EQUAL,
    ...
    case TokenType::GREATER_EQUAL:
      ...
      break;
    default:
      throw CompilerException::parser_error("Invalid comparison operator",
                                            op.line, op.column);
    }
...
ExprPtr Parser::parse_term() {
  ...
  while (match_one_of({TokenType::PLUS, TokenType::MINUS})) {
    ...
    BinaryExpr::Op bin_op = (op.type == TokenType::PLUS) ? BinaryExpr::Op::ADD
                                                         : BinaryExpr::Op::SUB;
    left =
        std::make_unique<BinaryExpr>(std::move(left), bin_op, std::move(right));
  }
...
ExprPtr Parser::parse_factor() {
  ...
  while (match_one_of(
    ...
    case TokenType::MODULO:
      ...
      break;
    default:
      throw CompilerException::parser_error("Invalid factor operator", op.line,
                                            op.column);
    }
...
ExprPtr Parser::parse_unary() {
  if (match_one_of({TokenType::MINUS, TokenType::NOT})) {
    ...
    UnaryExpr::Op un_op =
        (op.type == TokenType::MINUS) ? UnaryExpr::Op::NEG : UnaryExpr::Op::NOT;
  ...
  while (true) {
    if (match(TokenType::LPAREN)) {
      ...
      if (match(TokenType::LPAREN)) {
        ...
        expr = std::unique_ptr<MemberCallExpr>(
            new MemberCallExpr(std::move(expr), member.lexeme, {}, false));
      }
...
ExprPtr Parser::parse_primary() {
  ...
  if (match(TokenType::LBRACE)) {
    // Dictionary literal: {"key": "value", "num": 42} or {key: "value", num:
    // 42}
    std::vector<std::pair<ExprPtr, ExprPtr>> elements;

    if (!check(TokenType::RBRACE)) {
      do {
        // Check if the key is an identifier (for shorthand {name: value}
        // syntax)
        ExprPtr key;
        if (match(TokenType::IDENTIFIER)) {
...
void Parser::error(const std::string &message) {
  Token token = peek();
  throw CompilerException(ErrorType::PARSER_ERROR, message, token.line,
                          token.column);
}
...
std::string Parser::parse_type_hint() {
  ...
  if (match(TokenType::COLON)) {
    ...
    // This could be a simple identifier like "int" or a more complex type like
    // "Array[int]"
    ...
    // This allows for things like "var x:" which we'll just treat as no type
    // hint
  }
  return "";
}
