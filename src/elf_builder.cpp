...
std::vector<uint8_t> ElfBuilder::build(const IRProgram &program) {
  ...
  // Build symbol table
  // NOTE: Keep this struct defined here to avoid any issues with
  // alignment/packing
  ...
  for (size_t i = 0; i < program.functions.size(); i++) {
    ...
    // Calculate function size (distance to next function or end of text
    // section)
  ...
  shdr_shstrtab.sh_name =
      static_cast<uint32_t>(section_name_offsets[shstrtab_idx]);
...
void ElfBuilder::write_elf_header(std::vector<uint8_t> &data,
                                  uint64_t entry_point) {
  // Deprecated - now handled in build()
}

std::vector<uint8_t>
ElfBuilder::generate_minimal_code(const IRProgram &program) {
