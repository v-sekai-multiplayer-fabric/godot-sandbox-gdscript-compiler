...
// This represents a simplified, linear instruction stream that can be easily
// converted to RISC-V
...
enum class IROpcode {
  ...
  MAKE_PACKED_INT32_ARRAY, // Construct PackedInt32Array (empty or with
                           // elements)
  MAKE_PACKED_INT64_ARRAY, // Construct PackedInt64Array (empty or with
                           // elements)
  MAKE_PACKED_FLOAT32_ARRAY, // Construct PackedFloat32Array (empty or with
                             // elements)
  MAKE_PACKED_FLOAT64_ARRAY, // Construct PackedFloat64Array (empty or with
                             // elements)
  MAKE_PACKED_STRING_ARRAY,  // Construct PackedStringArray (empty or with
                             // elements)
  MAKE_PACKED_VECTOR2_ARRAY, // Construct PackedVector2Array (empty or with
                             // elements)
  MAKE_PACKED_VECTOR3_ARRAY, // Construct PackedVector3Array (empty or with
                             // elements)
  MAKE_PACKED_COLOR_ARRAY,   // Construct PackedColorArray (empty or with
                             // elements)
  MAKE_PACKED_VECTOR4_ARRAY, // Construct PackedVector4Array (empty or with
                             // elements)
...
struct IRInstruction {
  ...
  IRInstruction(IROpcode op, IRValue a, IRValue b)
      : opcode(op), operands{a, b} {}
  IRInstruction(IROpcode op, IRValue a, IRValue b, IRValue c)
      : opcode(op), operands{a, b, c} {}
...
inline bool is_vector(IRInstruction::TypeHint hint) {
  return hint == Variant::VECTOR2 || hint == Variant::VECTOR3 ||
         hint == Variant::VECTOR4 || hint == Variant::VECTOR2I ||
         hint == Variant::VECTOR3I || hint == Variant::VECTOR4I;
}

// Check if TypeHint is an integer vector type
inline bool is_int_vector(IRInstruction::TypeHint hint) {
  return hint == Variant::VECTOR2I || hint == Variant::VECTOR3I ||
