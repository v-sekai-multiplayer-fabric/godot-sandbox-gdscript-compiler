...
IRFunction compile_to_ir(const std::string &source,
                         const std::string &function_name = "test") {
  Lexer lexer(source);
...
std::string ir_to_string(const IRFunction &func) {
  ...
  for (size_t i = 0; i < func.instructions.size(); i++) {
    ...
    case IROpcode::LOAD_FLOAT_IMM:
      ...
      break;
    case IROpcode::MOVE:
      ss << "MOVE r" << std::get<int>(instr.operands[0].value) << ", r"
         << std::get<int>(instr.operands[1].value);
      break;
    case IROpcode::ADD:
    case IROpcode::SUB:
    case IROpcode::MUL:
    ...
    case IROpcode::MOD: {
      ...
      ss << op_name << " r" << std::get<int>(instr.operands[0].value) << ", r"
         << std::get<int>(instr.operands[1].value) << ", r"
         << std::get<int>(instr.operands[2].value);
      break;
    }
    default:
...
void test_pattern_a_basic() {
  std::cout
      << "Testing Pattern A (MOVE; MOVE; OP; MOVE with two temporaries)..."
      << std::endl;

  // Pattern A: MOVE tmp1, src1; MOVE tmp2, src2; OP dst, tmp1, tmp2; MOVE
  // result, dst
  //          -> OP result, src1, src2
  std::string source = R"(
func test(a, b):
  ...
  std::cout << "  MOVEs: " << move_count_no_opt << " -> " << move_count_opt
            << std::endl;
  std::cout << "  ADDs: " << add_count_no_opt << " -> " << add_count_opt
            << std::endl;
...
void test_pattern_b_operand1() {
  std::cout
      << "Testing Pattern B (MOVE; OP; MOVE with first operand temporary)..."
      << std::endl;
...
void test_pattern_c_operand2() {
  std::cout
      << "Testing Pattern C (MOVE; OP; MOVE with second operand temporary)..."
      << std::endl;
...
void test_pattern_d_move_after_op() {
  std::cout << "Testing Pattern D (OP; MOVE without preceding MOVE)..."
            << std::endl;
...
void test_pattern_e_increment() {
  std::cout << "Testing Pattern E (increment optimization: x = x + 1)..."
            << std::endl;
...
func test(x):
  ...
  std::cout << "  MOVEs: " << move_count_before << " -> " << move_count_after
            << std::endl;
  std::cout << "  LOAD_IMM: " << load_imm_count_before << " -> "
            << load_imm_count_after << std::endl;
  std::cout << "  ADDs: " << add_count_before << " -> " << add_count_after
            << std::endl;
  ...
  assert(move_count_after < move_count_before &&
         "Pattern E should reduce MOVEs");
  assert(load_imm_count_after <= load_imm_count_before &&
         "Pattern E should keep LOAD_IMM");
  assert(add_count_after == add_count_before &&
         "Pattern E should keep ADD count");

  std::cout << "  ✓ Pattern E test passed (reduced "
            << (move_count_before - move_count_after) << " MOVEs)" << std::endl;
}
...
func test(x):
  ...
  int move_count_before = count_instructions(func, IROpcode::MOVE);
  int load_float_count_before =
      count_instructions(func, IROpcode::LOAD_FLOAT_IMM);
  ...
  std::cout << "  MOVEs: " << move_count_before << " -> " << move_count_after
            << std::endl;
  std::cout << "  LOAD_FLOAT_IMM: " << load_float_count_before << " -> "
            << load_float_count_after << std::endl;

  assert(move_count_after < move_count_before &&
         "Pattern E should reduce MOVEs for floats");
...
func test(a):
  ...
  std::cout << "  Before optimization: " << move_count_before << " MOVEs"
            << std::endl;
  ...
  int move_count_after = count_instructions(func, IROpcode::MOVE);
  std::cout << "  After optimization: " << move_count_after << " MOVEs"
            << std::endl;
...
func test():
  ...
  std::cout << "  Final: " << load_imm_count << " LOAD_IMM, " << add_count
            << " ADD, " << move_count << " MOVE" << std::endl;

  assert(add_count == 0 && "Constant folding should eliminate ADD");
  assert(load_imm_count == 1 &&
         "Constant folding should result in single LOAD_IMM");

  std::cout << "  ✓ Constant folding test passed" << std::endl;
}

void test_combined_optimizations() {
  std::cout << "Testing combined optimizations (loop with increment)..."
            << std::endl;
...
func test():
  ...
  std::cout << "  Before optimization: " << move_count_before << " MOVEs, "
            << add_count_before << " ADDs" << std::endl;
  ...
  std::cout << "  After optimization: " << move_count_after << " MOVEs, "
            << add_count_after << " ADDs" << std::endl;
  std::cout << "  Reduced " << (move_count_before - move_count_after)
            << " MOVEs" << std::endl;
...
func test():
  ...
  std::cout << "  Max registers before optimization: " << func.max_registers
            << std::endl;

  IROptimizer optimizer;
  optimizer.optimize_function(func);

  std::cout << "  Max registers after optimization: " << func.max_registers
            << std::endl;
