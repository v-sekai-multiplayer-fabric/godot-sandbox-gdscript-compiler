...
void test_comparison_operators() {
  ...
  int cmp_count = 0;
  for (const auto &instr : ir.functions[0].instructions) {
    if (instr.opcode == IROpcode::CMP_EQ || instr.opcode == IROpcode::CMP_NEQ ||
        instr.opcode == IROpcode::CMP_LT || instr.opcode == IROpcode::CMP_LTE ||
        instr.opcode == IROpcode::CMP_GT || instr.opcode == IROpcode::CMP_GTE) {
      cmp_count++;
    }
  }
  ...
  for (const auto &instr : ir_read.functions[0].instructions) {
    if (instr.opcode == IROpcode::VCALL) {
      if (instr.operands.size() >= 3 &&
          instr.operands[2].type == IRValue::Type::STRING) {
...
void test_array_dictionary_constructors() {
  std::cout << "Testing Array and Dictionary constructor generation..."
            << std::endl;
...
void test_many_float_constants() {
  std::cout << "Testing many float constants (FP register exhaustion)..."
            << std::endl;

  // Create more float constants than available FP registers (12 temp FP regs:
  // f8-f19)
  ...
  try {
    ...
    std::cerr << "    Note: Code generation issue with many floats: "
              << e.what() << std::endl;
  }
...
void test_auipc_addi_patching() {
  std::cout << "Testing AUIPC+ADDI label patching (many constants)..."
            << std::endl;
  ...
  try {
    ...
    for (size_t i = 0; i < code.size() - 3; i += 4) {
      uint32_t instr = code[i] | (code[i + 1] << 8) | (code[i + 2] << 16) |
                       (code[i + 3] << 24);
    ...
    // (Note: this might not always trigger depending on constant values, but we
    // check the code gen doesn't crash)
  } catch (const CompilerException &e) {
    std::cerr << "    Note: AUIPC+ADDI test encountered issue: " << e.what()
              << std::endl;
  }
...
void test_constant_fold_comparison_in_if() {
  std::cout << "Testing constant folding of comparisons in if statements..."
            << std::endl;
  ...
  // and the BRANCH_ZERO should be removed (or replaced with unconditional
  // branch)
