...
CompilationResult
compile_with_register_info(const std::string &source,
  ...
  // Note: The allocator processes each function, so we get the state after all
  // functions
...
func many_variables():
  ...
  CompilationResult result =
      compile_with_register_info(source, "many_variables");
...
void test_complex_expression_no_unnecessary_spill() {
  std::cout << "Testing register allocation with complex expressions..."
            << std::endl;
...
void test_register_allocation_no_unnecessary_spills() {
  std::cout << "Testing register allocation avoids unnecessary spills..."
            << std::endl;
...
void test_register_allocation_minimal_spilling() {
  std::cout << "Testing codegen handles many variables correctly..."
            << std::endl;
...
func test_25_vars():
  ...
  assert(result.max_registers >= VARIABLES &&
         "Should allocate at least stack slots for all variables");

  std::cout << "  ✓ Codegen handles many variables correctly" << std::endl;
}

void test_register_allocation_never_exceeds_limit() {
  std::cout << "Testing codegen handles variable counts correctly..."
            << std::endl;
...
func test_exactly_18():
  ...
  CompilationResult result_18 =
      compile_with_register_info(source_18, "test_exactly_18");
...
func test_19_vars():
  ...
  CompilationResult result_19 =
      compile_with_register_info(source_19, "test_19_vars");

  // In current implementation, all Variants are stack-allocated
  assert(result_18.max_registers >= 18 &&
         "Should allocate at least enough stack slots");
  assert(result_19.max_registers >= 19 &&
         "Should allocate at least enough stack slots");
...
void test_edge_case_only_parameters() {
  std::cout << "Testing edge case: function with only parameters..."
            << std::endl;
...
void test_edge_case_many_variables_stress() {
  std::cout
      << "Testing edge case: very large number of variables (stress test)..."
      << std::endl;
...
int main() {
  ...
  std::cout << "These tests verify the full compilation pipeline and register "
               "allocation\n"
            << std::endl;
