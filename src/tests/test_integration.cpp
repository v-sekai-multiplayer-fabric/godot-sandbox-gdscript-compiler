...
IRInterpreter::Value
execute(const std::string &source, const std::string &function = "main",
        const std::vector<IRInterpreter::Value> &args = {}) {
  // Compile to IR and execute (skip ELF generation for now, since RISC-V
  // backend has limitations)
  Lexer lexer(source);
...
int64_t execute_int(const std::string &source,
...
func main():
  ...
  std::cout << "  ✓ Simple return test passed (result: " << result << ")"
            << std::endl;
}
...
func main():
  ...
  std::cout << "  ✓ Variable operations passed (result: " << result << ")"
            << std::endl;
}
...
func factorial(n):
  ...
  assert(execute_int(source, "sum_to_n", {int64_t(10)}) ==
         55); // 1+2+...+10 = 55
...
func fibonacci(n):
  ...
  std::cout
      << "  ⚠ Fibonacci test skipped (recursive calls not yet implemented)"
      << std::endl;
...
func compute(a, b, c):
  ...
  assert(execute_int(source, "compute",
                     {int64_t(10), int64_t(5), int64_t(2)}) == 28);
...
func factorial(n):
  ...
  std::cout << "  ✓ Generated ELF file: /tmp/test.elf (" << elf_data.size()
            << " bytes)" << std::endl;
  std::cout << "  ✓ Functions: test, sum_to_n, count_down, factorial"
            << std::endl;
...
func test_division_rounding():
  ...
  assert(execute_int(source, "test_multiple_operations") ==
         26); // (10+5-2)*2 = 26
...
func test_multiplication_table():
  ...
  assert(execute_int(source, "test_multiplication_table") ==
         225); // sum of 5x5 multiplication table
...
func test():
  ...
  std::cout << "Opcodes: LOAD_IMM=0, MOVE=5, ADD=6, CMP_LT=11, LABEL=18, "
               "JUMP=19, BRANCH_ZERO=20"
            << std::endl;
  for (const auto &func : ir.functions) {
    if (func.name == "test") {
      std::cout << "Function has " << func.max_registers << " max registers"
                << std::endl;
...
void test_for_loop_new_variable() {
  std::cout << "Testing for loop with new variable declaration in body..."
            << std::endl;
...
func complex_calculation(w, x, y, z):
  ...
  assert(execute_int(source_basic, "add_three",
                     {int64_t(1), int64_t(2), int64_t(3)}) == 6);
  assert(execute_int(source_basic, "multiply_and_add",
                     {int64_t(3), int64_t(4), int64_t(5)}) == 17);
  assert(execute_int(source_basic, "complex_calculation",
                     {int64_t(20), int64_t(10), int64_t(3), int64_t(5)}) ==
         15); // 30-15

  std::cout << "  ✓ Function calls with multiple arguments passed" << std::endl;
  std::cout
      << "  ℹ Note: Inter-function calls not fully implemented (CALL opcode)"
      << std::endl;
}
...
func combined_conditions(a, b):
  ...
  assert(execute_int(source, "combined_conditions",
                     {int64_t(5), int64_t(10)}) == 1);
  assert(execute_int(source, "combined_conditions",
                     {int64_t(-5), int64_t(-10)}) == -1);
  assert(execute_int(source, "combined_conditions",
                     {int64_t(5), int64_t(-10)}) == 0);
...
func digit_sum(n):
  ...
  assert(execute_int(source, "sum_of_squares", {int64_t(5)}) ==
         55); // 1+4+9+16+25
...
func exponential_growth():
  ...
  assert(execute_int(source, "exponential_growth") ==
         7); // 1,2,4,8,16,32,64 (stops before 128)
...
func double_add(x, y):
  ...
  assert(execute_int(source_simple, "double_add", {int64_t(3), int64_t(4)}) ==
         14);
...
func process(x):
  ...
  assert(execute_int(source_chain, "process", {int64_t(4)}) ==
         17); // 4*3+5 = 17
...
func sum_of_squares(a, b):
  ...
  assert(execute_int(source_multiple, "sum_of_squares",
                     {int64_t(3), int64_t(4)}) == 25); // 9+16
...
func complex(x, y):
  ...
  assert(execute_int(source_nested, "complex", {int64_t(5), int64_t(2)}) ==
         16); // 5*2 + 2*3 = 16
...
int main() {
  ...
  std::cout
      << "These tests compile GDScript and execute it via IR interpreter\n"
      << std::endl;
  ...
  try {
    ...
    std::cout << "✅ Basic features: arithmetic, variables, conditionals"
              << std::endl;
    std::cout << "✅ Control flow: while loops, for loops, break, continue"
              << std::endl;
    std::cout << "✅ Variable scoping with shadowing works correctly"
              << std::endl;
    std::cout << "✅ Real-world algorithms: Fibonacci, GCD, Prime check, "
                 "Collatz, Power"
              << std::endl;
    std::cout << "✅ Edge cases: zero iterations, countdown, nested loops"
              << std::endl;
    std::cout << "✅ Function calls with multiple arguments" << std::endl;
    std::cout << "✅ Complex conditional logic with nested if/elif/else"
              << std::endl;
    return 0;
