...
void test_simple_function() {
  ...
  auto *ret_stmt =
      dynamic_cast<ReturnStmt *>(program.functions[0].body[0].get());
...
void test_while_loop() {
  ...
  auto *while_stmt =
      dynamic_cast<WhileStmt *>(program.functions[0].body[1].get());
...
void test_expressions() {
  ...
  for (int i = 0; i < 4; i++) {
    auto *var_decl =
        dynamic_cast<VarDeclStmt *>(program.functions[0].body[i].get());
...
void test_function_call() {
  ...
  auto *var_decl =
      dynamic_cast<VarDeclStmt *>(program.functions[0].body[0].get());
  assert(var_decl != nullptr);
...
void test_method_call() {
  ...
  auto *expr_stmt =
      dynamic_cast<ExprStmt *>(program.functions[0].body[1].get());
  assert(expr_stmt != nullptr);

  auto *member_call =
      dynamic_cast<MemberCallExpr *>(expr_stmt->expression.get());
