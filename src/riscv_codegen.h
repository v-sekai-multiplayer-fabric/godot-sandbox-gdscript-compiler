...
public:
  ...
  const std::unordered_map<std::string, size_t> &get_function_offsets() const {
    return m_functions;
  }
  ...
  const std::vector<int64_t> &get_constant_pool() const {
...
private:
  ...
  void emit_r_type(uint8_t opcode, uint8_t rd, uint8_t funct3, uint8_t rs1,
                   uint8_t rs2, uint8_t funct7);
  void emit_i_type(uint8_t opcode, uint8_t rd, uint8_t funct3, uint8_t rs1,
                   int32_t imm);
  void emit_s_type(uint8_t opcode, uint8_t funct3, uint8_t rs1, uint8_t rs2,
                   int32_t imm);
  void emit_b_type(uint8_t opcode, uint8_t funct3, uint8_t rs1, uint8_t rs2,
                   int32_t imm);
  void emit_u_type(uint8_t opcode, uint8_t rd, uint32_t imm);
  void emit_j_type(uint8_t opcode, uint8_t rd, int32_t imm);
  void emit_r4_type(uint8_t opcode, uint8_t rd, uint8_t funct3, uint8_t rs1,
                    uint8_t rs2, uint8_t rs3, uint8_t funct2);

  // Higher-level RISC-V instructions
  void emit_li(uint8_t rd, int64_t imm); // Load immediate
  void emit_la(uint8_t rd,
               const std::string &label); // Load address (pseudo: auipc + addi)
  ...
  void emit_seqz(uint8_t rd,
                 uint8_t rs); // Set if equal to zero (pseudo: sltiu rd, rs, 1)
  void
  emit_snez(uint8_t rd,
            uint8_t rs); // Set if not equal to zero (pseudo: sltu rd, x0, rs)
  ...
  void emit_ld(uint8_t rd, uint8_t rs1,
               int32_t offset); // Load doubleword (64-bit)
  void emit_lw(uint8_t rd, uint8_t rs1, int32_t offset); // Load word (32-bit)
  void emit_lwu(
      uint8_t rd, uint8_t rs1,
      int32_t offset); // Load word unsigned (32-bit, zero-extended to 64-bit)
  void emit_lh(uint8_t rd, uint8_t rs1,
               int32_t offset); // Load halfword (16-bit)
  void emit_lb(uint8_t rd, uint8_t rs1,
               int32_t offset); // Load byte (8-bit signed)
  void emit_lbu(uint8_t rd, uint8_t rs1,
                int32_t offset); // Load byte unsigned (8-bit)
  void emit_sd(uint8_t rs2, uint8_t rs1,
               int32_t offset); // Store doubleword (64-bit)
  void emit_sw(uint8_t rs2, uint8_t rs1, int32_t offset); // Store word (32-bit)
  void emit_sh(uint8_t rs2, uint8_t rs1,
               int32_t offset); // Store halfword (16-bit)
  void emit_sb(uint8_t rs2, uint8_t rs1, int32_t offset); // Store byte (8-bit)

  // Floating-point load/store (RV64D extension)
  void emit_fld(uint8_t rd, uint8_t rs1,
                int32_t offset); // Load double (64-bit FP)
  void emit_fsd(uint8_t rs2, uint8_t rs1,
                int32_t offset); // Store double (64-bit FP)
  void emit_flw(uint8_t rd, uint8_t rs1,
                int32_t offset); // Load float (32-bit FP)
  void emit_fsw(uint8_t rs2, uint8_t rs1,
                int32_t offset);               // Store float (32-bit FP)
  ...
  void emit_fcvt_d_l(uint8_t rd,
                     uint8_t rs1); // Convert signed 64-bit int to double

  // FP arithmetic instructions (RV64D extension - double precision)
  void emit_fadd_d(uint8_t rd, uint8_t rs1,
                   uint8_t rs2); // Double-precision FP add
  void emit_fsub_d(uint8_t rd, uint8_t rs1,
                   uint8_t rs2); // Double-precision FP sub
  void emit_fmul_d(uint8_t rd, uint8_t rs1,
                   uint8_t rs2); // Double-precision FP mul
  void emit_fdiv_d(uint8_t rd, uint8_t rs1,
                   uint8_t rs2);           // Double-precision FP div
  ...
  void emit_fadd_s(uint8_t rd, uint8_t rs1,
                   uint8_t rs2); // Single-precision FP add
  void emit_fsub_s(uint8_t rd, uint8_t rs1,
                   uint8_t rs2); // Single-precision FP sub
  void emit_fmul_s(uint8_t rd, uint8_t rs1,
                   uint8_t rs2); // Single-precision FP mul
  void emit_fdiv_s(uint8_t rd, uint8_t rs1,
                   uint8_t rs2); // Single-precision FP div

  // Additional integer instructions
  void
  emit_sext_w(uint8_t rd,
              uint8_t rs); // Sign-extend word to doubleword (addiw rd, rs, 0)
  ...
  void emit_load_variant_type(uint8_t rd, uint8_t base_reg,
                              int32_t variant_offset);
  void emit_store_variant_type(uint8_t rs, uint8_t base_reg,
                               int32_t variant_offset);
  void emit_load_variant_bool(uint8_t rd, uint8_t base_reg,
                              int32_t variant_offset);
  void emit_store_variant_bool(uint8_t rs, uint8_t base_reg,
                               int32_t variant_offset);
  void emit_load_variant_int(uint8_t rd, uint8_t base_reg,
                             int32_t variant_offset);
  void emit_store_variant_int(uint8_t rs, uint8_t base_reg,
                              int32_t variant_offset);
  ...
  void emit_variant_eval(int result_offset, int lhs_offset, int rhs_offset,
                         int op);

  // VCREATE syscall abstraction
  // Generic VCREATE emission that handles register clobbering and stack
  // management variant_type: Variant::Type enum value (e.g., Variant::STRING,
  // Variant::ARRAY) method: Constructor method number (type-specific)
  ...
  void emit_vcreate_syscall(int variant_type, int method, uint8_t data_ptr_reg,
                            int result_offset);
  ...
  void emit_typed_int_binary_op(int result_offset, int lhs_offset,
                                int rhs_offset, IROpcode op);
  void emit_typed_int_comparison(int result_offset, int lhs_offset,
                                 int rhs_offset, IROpcode cmp_op);
  void emit_typed_float_binary_op(int result_offset, int lhs_offset,
                                  int rhs_offset, IROpcode op);
  void emit_typed_vector_binary_op(int result_offset, int lhs_offset,
                                   int rhs_offset, IROpcode op,
  ...
  // Store syscall result from register to Variant, with optional register
  // allocation result_vreg: virtual register for the result result_reg:
  // physical register containing the result (e.g., REG_A0 after syscall)
  // result_offset: stack offset for the Variant
  // variant_type: Variant type value (e.g., 2 for INT, 24 for GDOBJECT)
  void emit_syscall_result(int result_vreg, uint8_t result_reg,
  ...
  int m_current_instr_idx =
      0; // Current instruction index for register allocation
  ...
  static constexpr int VARIANT_TYPE_OFFSET =
      0; // Offset to m_type field (4 bytes)
  static constexpr int VARIANT_DATA_OFFSET =
      8; // Offset to union data (bool/int64/etc)

  // String constants from IR
  const std::vector<std::string> *m_string_constants = nullptr;

  // Constant pool for large immediates (64-bit values that can't be encoded in
  // instructions)
  ...
  // dest_offset + store_offset Handles both INT and FLOAT Variants If
  // normalize_by_255 is true, divides integers by 255.0 (for Color components)
  void emit_variant_component_to_float(int comp_offset, int result_offset,
                                       int store_offset,
                                       bool normalize_by_255 = false);
